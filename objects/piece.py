    ...
    def generate_pseudo_legal_moves(self, board: GameBoard) -> list[Move]:
        ...
        for dh in [-1, 1]:
            ...
            if board.in_bounds((one_forward, new_col)) and not board.is_empty(
                ...
                if (possible_piece is not None) and self.is_enemy(possible_piece):
        ...
        for dh in [-1, 1]:
            ...
            if board.in_bounds((row, new_col)) and not board.is_empty((row, new_col)):
                if board.double_jumped_pawn == board.get_square_contents(
                    (row, new_col)
                ) and self.is_enemy(board.double_jumped_pawn):
                    pseudo_legal_moves.append(
                        Move(
                            self.pos,
                            (row + dv, new_col),
    ...
    def generate_pseudo_legal_moves(self, board: GameBoard) -> list[Move]:
        ...
        for dr, dc in KNIGHT_OFFSET:
            ...
            if board.in_bounds(
                ...
                if (possible_piece is None) or self.is_enemy(possible_piece):
                    pseudo_legal_moves.append(
                        Move(self.pos, (new_row, new_col), MOVE_NORMAL, None)
                    )
        return pseudo_legal_moves
            ...
            if self.ks_rook.is_active and not self.ks_rook.has_moved:
                if board.squares_between_empty(self, self.ks_rook):
                    if board.path_safe(self, self.ks_rook):
                        ...
                        pseudo_legal_moves.append(
                            Move(self.pos, king_to_square, MOVE_CASTLE, payload)
                        )
